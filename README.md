...
| v1.11 | 新規ファイル テンプレート (内蔵「空のファイル」+ ユーザー定義) / テーマ プリセット 9 種 / アイコン パック 5 種 / .iso・.vhd ダブルクリックでマウント | ✅ |
| v1.12 | シェル統合 (Excel リンク等から新規タブで開く / フォルダ既定ハンドラ トグル / コマンドライン引数で起動) | ✅ |
| v1.13 | ユーザー定義コマンド (`commands.json` で右クリックメニューに 7-Zip / VSCode / 自作スクリプト等を追加) | ✅ |
| v1.14 | アイコン パック改善 (Windows シェル アイコン `system` / SVG カスタム パック `custom:<id>` / 同梱 Material パック / `%APPDATA%\fastfiler\icons\` の `manifest.json` で拡張) | ✅ |
