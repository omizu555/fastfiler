...
- **v3.0 ドック**: パネルヘッダの ``◧ ◨ ⬒ ⬓ ✕`` ボタンでタブ / ツリーを **左 / 右 / 上 / 下 / 非表示** に独立ドック可能。同 slot に複数置いた場合の並列/積み重ねは設定で切替 (詳細: [`doc/USAGE.md`](./doc/USAGE.md) §27)。
...
- **エクスプローラ等から FastFiler へドロップ**: 受信側を OLE D&D で実装 (Shift=移動 / Ctrl=コピー / Alt=ショートカット)
- **Alt + ドラッグ**: FastFiler からエクスプローラ等の他アプリへネイティブ OS ドラッグ (drag-out)

### Windows シェル統合 (v4.0)
- 右クリックメニュー末尾の「🪄 Windows メニュー…」または **Shift + 右クリック** で、Windows のシェル拡張 (圧縮、Git、7-Zip 等) のネイティブメニューを表示。
