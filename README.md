...
### プラグイン
- `%APPDATA%\fastfiler\plugins\<id>\manifest.json` を置くと自動検出。
- WebView 内で `parent.postMessage({ __ff: "invoke", id, capability, args })` を呼ぶと、capability に許可された範囲で API を実行可能（例: `fs.read.dir` / `fs.read.text` / `shell.open`）。
- サンプル: `doc/plugins-sample/hello-plugin/`。
...
│  ├ App.tsx, main.tsx, store.ts, fs.ts, types.ts
│  └ components\          # VerticalTabs / PaneTree / FileList / SettingsDialog /
│                         # ContextMenu / Thumbnail / PreviewPane / SearchPanel / PluginPanel
