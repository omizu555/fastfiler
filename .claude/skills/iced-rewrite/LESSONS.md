...
- update_overlay の `_ => Some(vec![])` (既定スワロー) は、キー横取り防止の
  ...
  **入力 UI 系オーバーレイ (PathEdit) は、マウス由来メッセージを
  「破棄 + None フォールスルー」にして 1 クリック目からその操作を効かせる**
  (エクスプローラのアドレスバー準拠。閉じるだけでもう 1 クリック要求しない)。
- この区別が安全に書ける根拠: オーバーレイ表示中のキーは GUI 層 (KeyPressed
  ハンドラ) で止まっているので、GoParent 等が reducer に届くのはボタン/マウス
  経由のときだけ — 「層をまたぐ不変条件」はコメントで残すこと。
- フォーカス移動でペインを離れるときは移動元の入力オーバーレイを破棄する。
  残すと不可視のまま持ち越され、戻ったとき古い編集状態が復活する。
...
- .lnk に明示 verb ("open") を渡すと、リンク解決後の**ターゲットにも同じ verb が
  適用される** — Office テンプレートのショートカットが「テンプレートから新規
  (既定 verb "new")」でなく編集モードで開く原因になった。ショートカットは
  IShellLinkW で自前解決してターゲットの拡張子で verb を選び直す
  (shell.rs resolve_shortcut / shell_verb_for)。引数・作業フォルダも
  リンクから引き継ぐ (エクスプローラの流儀)。
- 「拡張子 → verb」の判断は既に 2 系統あった (テンプレート=None / iso=mount)。
  この種の分岐は関数 (shell_verb_for) に集約しておくと .lnk のような
  「間接参照の再判定」で再利用できる。

## 2026-07-08 「起動時に cmd っぽい窓が見える」の切り分けと修正

- **Windows の一瞬の窓の正体調査は EnumWindows の 10ms ポーリングが決定打**:
  クラス名 + PID + 矩形が取れるので「コンソールか / 自分の窓か / どの位置か」を
  機械的に区別できる (スクリーンショット連写は 4K で 1 枚 ~150ms かかり取り逃す)。
- **Win11 のコンソールは conhost ではなく Windows Terminal がホスト**
  (CASCADIA_HOSTING_WINDOW_CLASS)。conhost.exe だけ監視しても検出できない。
- **iced/winit は既定でウィンドウを「可視」で生成する** → 初回描画前の黒い
  クライアント領域が既定サイズ・画面中央に一瞬見え、その後セッション位置へ
  ジャンプする (ダークテーマだと cmd 窓に見える)。対策: `visible: false` で生成し、
  WindowOpened で move_to/resize/maximize を適用した最後に
  `window::set_mode(id, Mode::Windowed)` で表示する (0.14 に set_visible は無い —
  set_mode が可視切替を兼ねる)。
- **`cargo build … | tail` はリンク失敗を隠す** — パイプの exit code は tail のもの。
  実行中 exe (自分の検証インスタンス含む) が release リンクと衝突すると
  os error 5 で失敗するが、握り潰されて成功に見えた。ビルド検証は
  `PIPESTATUS[0]` か生 exit code で確認し、**検証起動とビルドを同時にしない**。

## 2026-07-07 UI テストの座標決定

- **2D 総当たりの座標プローブは使わない**: view 構築 × 数百回で 69s かかった上に、
  ステップ幅が対象 (高さ 24px のツリー行 0) を跨いで外した。座標が 1 軸だけ不明なら
  細かいステップの 1 軸プローブ (file_list の先例、~20 回) で良いが、2 軸不明なら
  **レイアウト式をテスト用フック (app.rs の *_for_test) に写して決め打ち**する
  (tree_row0_center_for_test が先例。0.35s で安定)。フックはレイアウト式と
  対で保守する旨をコメントに残す。

## 2026-07-06 パスの二重区切り (D:\\AI) の潜伏バグ

- **Path の Eq/Ord/Hash は components ベースで「連続区切り・末尾区切り」を吸収するが、
  to_string_lossy の文字列比較と画面表示は吸収しない**。この非対称のせいで
  二重区切りは「動くのに表示が変」という形で潜伏する (Windows API も `\\` を許容)。
  正典パス (cur_path 等) はチョークポイント (コンストラクタ + ナビゲーション) で
  `path.components().collect::<PathBuf>()` の正規化を通す — スラッシュ入力・
  末尾区切りも同時に直り、UNC / verbatim prefix は不変。
- **domain の DriveInfo.letter は "C:\" 形式 (末尾 \ 付き)**。"C:" 前提で
  `format!("{letter}\\")` すると "C:\\" になる (今回の発生源。tree.rs の
  ドキュメントコメントは "C:" 例で、seam の前提ズレだった)。

## 2026-07-05 実機フィードバック 3 件 (複数選択ドラッグ / 偽パス / フッター)

- **押下 (Pressed) で選択を確定すると複数選択の左ドラッグが成立しない**:
  エクスプローラ系 UI の作法は「選択済み行の修飾なし押下は選択を維持し、
  ドラッグに至らなければ Release で単一選択へ確定」。widget は生イベント
  (Pressed / Released / DragStarted) を流すだけにし、確定タイミングの判断は
  core の pending 状態 (pending_click) に置くと単体テストできる
  (右ボタンの RightPressed が既に同じ発想だった — 左にも展開)。
- **楽観ナビゲーション (cur_path を先に書き換えて読む方式) には「表示中一覧の
  実パス (loaded_path)」を対で持たせる**: 読み込み失敗時にそこへ復帰しないと
  「一覧は旧フォルダ・パスだけ偽」の合成状態になり、行の join 解決が
  偽パスを増殖させる (実機報告: パス入力ミス後のダブルクリックでパスが伸びる)。
  失敗復帰では直前のナビゲーションが積んだ履歴 (back/fwd の top) の掃除も対にする。
- UI 操作の再現テストは ui_smoke.rs の「座標プローブ → simulate → メッセージを
  update へ適用」パターンが速い。プローブは view の複製に対して行えば状態を汚さない。
