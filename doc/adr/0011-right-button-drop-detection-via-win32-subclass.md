...
- 状態: **Superseded** (2026-06-07 — [ADR 0012](./0012-migrate-floem-to-gpui.md) の GPUI 移行で不要化。
  本決定は floem 0.2 のイベント制約への対処であり、GPUI は右ボタンイベントを
  ネイティブに扱えるためサブクラスは実装されていない。Win32 フック類のコードも現存しない)
- 旧状態: 採用 (2026-05-24)
