...
- 「旧 floem 版は当面ワークスペースに残す」→ **削除済み**
  (`crates/fastfiler-native` は存在しない。履歴: `wip(floem): メモリ増殖調査の計装を保全` 以前)。
- 上の「未対応 (今後)」リストは**ほぼ全て実装済み**:
  外部送信 (右ボタン含む) / フォルダ行への直接ドロップ / UNC サーバノード /
  ユーザーコマンド / Shift+右クリックシェルメニュー / テーマ・スタイル・フォント設定 /
  ホットキーカスタマイズ / exe アイコン / 多重起動防止。
  残: **ペイン内ツリー** (未実装のまま)。現況は [`doc/README.md`](../README.md) を参照。

## 進捗記録

移植の経緯・実装メモは [`doc/plan/2026-06-03-gpui-migration.md`](../plan/2026-06-03-gpui-migration.md) §11 を参照。
