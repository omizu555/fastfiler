...
最終更新: 2026-05-23 / **v0.1.0 リリース候補**

> **v0.1.0 リリースマイルストーン**
> 旧 TARUI (Tauri 2 + Solid.js) 版で実装していた中核機能を floem 単独構成で
> ほぼ再現完了 (タブ / 任意分割ペイン / D&D / Undo / 検索 / テンプレ / Spring-loaded
> / プログレス / パフォーマンス計測 / ツリーペイン)。当面の開発を一旦止め、
> 配布用リリースを作成する段階。
...
- [x] リネーム / 新規ファイル / 新規フォルダ (センターポップアップダイアログ)
...
- [x] 新規ファイルテンプレート (`%APPDATA%\FastFiler\templates\` のユーザー定義ファイル
      を右クリック「新規ファイル ▸」サブメニューから選択して複製作成。同名衝突時は `(2)` を自動付与)
...
### ペイン詳細ビュー (チューニング)
- [x] 列幅ドラッグ可変 (名前 / サイズ / 更新日時) — ハンドル幅 7px・ドラッグ中ハイライト
- [x] 名前列の省略表示 (`…`) を UI フォントサイズ比例の実測幅ベースで判定 (ASCII fs×0.55 / 全角 fs×1.05 / ellipsis fs×0.6)
...
- [ ] **パネルプリセット** (レイアウト + 開いているタブをまとめて保存・呼出)
      — ドック 5 ヶ所化 (上記) が前提
- [x] **Spring-loaded folder** (D&D 中に 0.7 秒ホバーで自動展開) — ペイン内フォルダ行は cd、ツリーノードは expand。内部 D&D 専用
