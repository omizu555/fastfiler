...
### 2026-06-05 — 改善フェーズ計画 (ユーザー要望 + 質疑で確定)
着手順: **B-1 → B-2 → A**
- **B-1 同名衝突ダイアログ**: run_transfer 入口で衝突検査。衝突あり →
  「N 件が既に存在します」**[上書き] [別名で保存] [キャンセル] (一括適用)**。
  別名 = 衝突分のみ `name (2).ext` 自動連番。全転送経路 (貼り付け/内部D&D/
  外部D&D/フォルダ行/右ドラッグチューザー) に 1 箇所で効く。
  ※現状は黙って上書き (copy=truncate / move=rename失敗時に上書きコピー+削除)。
- **B-2 外部 D&D の修飾キー**: ドロップ時 Ctrl=コピー / Shift=移動 /
  なし=**エクスプローラ準拠** (同一ドライブ=移動・別ドライブ=コピー、現行どおり)。
  内部 D&D にも同じ修飾キーを適用 (一貫性)。
- **A ヘッダー + 設定画面**: 全幅ヘッダー (左タイトル / 右端 ⚙) + 中央モーダルの
  設定画面。項目: **テーマ選択 (一覧) / ホットキー (開く・再読込) / フォルダ類
...
### 2026-06-05 — タブバー列数設定 ✅
- 設定に **「タブバー — 列数」(1〜4)** を追加 (`settings.tab_columns`、既定 1)。
- タブは**行優先**で並べる (2 列なら 1,2 / 3,4 / 5,6)。端数行は空きスロットで埋め。
- バー幅は列数に応じて拡張 (1 列あたり最低 100px、1〜2 列は従来の 200px)。
- 変更は即時反映 (cx.notify で再描画) + gpui_settings.json へ保存。

### 2026-06-05 — タブバー幅リサイズ + タブ D&D 並べ替え ✅
- **幅リサイズ**: タブバー右端に 5px ハンドル (ツリーと同じ方式 —
  `DraggedTabBarHandle` + 行コンテナの on_drag_move で左端起点の幅を算出)。
  clamp 100〜600px、列数ぶんの最低幅 (80px/列) は確保。session に `tab_width` 保存。
- **並べ替え**: タブタイトルを gpui 内部ドラッグ (`DraggedTab{ix}` +
  タイトルチップのプレビュー) → 他タブへドロップで `move_tab(from, to)`。
  drag_over で drop_bg ハイライト。active index は移動に追従。順序は session 保存
  (タブ配列順そのまま)。

### 2026-06-05 — タブ名 1 行固定 / ツリー追従 / テーマコンボ ✅
- **タブ名**: `.truncate()` で折返し禁止 + はみ出しは … 省略 (1 行固定)。
- **ツリー追従**: `TreeView::reveal(path)` — 祖先を展開し対象行を選択ハイライト
  (sel_bg) + `scroll_to_item(Center)`。パス照合は大文字小文字無視・ツリー側表記で
  辿る (隠しフォルダ等で辿れない場合は最深の祖先で停止)。
  発火点: ペイン observe (フォルダ移動) / select_tab / set_focus / cycle_focus /
  close_tab / toggle_tree (再表示時) / 起動時。`last_revealed` で重複抑止。
- **テーマ選択をコンボボックス化**: 設定画面のテーマをボタン列 → 現在値 + ▼ の
  ドロップダウン (deferred + absolute、開閉トグル、選択で即適用・保存)。

### 2026-06-05 — パスバー/一覧の表示改善 ✅
- **戻る/進むボタンを非表示** (マウス第4/5ボタン・Alt+←→ は従来どおり動作)。
- **パス表示のはみ出し抑制**: `min_w_0 + truncate` で右側ボタン (↔ 等) へ
  はみ出さず … 省略。クリックで入力欄 (TextInput) になりマウスドラッグで
  部分選択可 (従来から対応済みの機能を明記)。
- **サイズ/種類列の整列**: 行・見出しとも名前セルに `min_w_0 + truncate`、
  固定幅セルに `flex_shrink_0` — 長いファイル名が列を押し出して
  行ごとに列位置がずれる問題を解消。

### 2026-06-06 — 検索バー改善 / タブバー整理 / 更新日時列 ✅
- **検索**: Ctrl+F でバーだけ表示し、**Enter で実行するまでファイル一覧を残す**
  (`SearchUi.executed` フラグで結果リストへの切替を遅延)。実行後は従来どおり。
- **タブバー**: 新規タブボタンを「＋」に短縮。「ツリー」トグルボタンは設定
  (`show_tree_button`、タブバーセクションの 表示/非表示 トグル) で出し分け。
- **更新日時列**: 名前とサイズの間に 118px 固定で追加。`format_modified` =
  unix 秒 → FILETIME → SystemTimeToTzSpecificLocalTime (windows 0.61 +
  Win32_System_Time feature) で "YYYY/MM/DD HH:MM"。見出しクリックで
  ソート対応 (`SortCol::Modified`)。

### 2026-06-06 — 更新日時列の修正 + 列幅リサイズ ✅
- **被り修正**: 118px では "YYYY/MM/DD HH:MM" が収まらず折返して隣へ被っていた。
  既定 132px + `whitespace_nowrap + overflow_hidden` で 1 行収め (時刻まで表示)。
- **列幅リサイズ**: 見出しの各固定列 (更新日時/サイズ/種類) 左端に 6px 仕切り
  ハンドル (`ColHandle{col}` + ペイン全域の on_drag_move)。右端基準で幅を逆算、
  clamp 40〜400px。設定 (`col_modified/col_size/col_type`) に即保存され全ペイン
  共通・再起動後も維持。狭くした分は名前列が広がる。
- **暴れ修正 (実機報告)**: `on_drag_move` は Capture フェーズで**カーソル位置に
  関係なく登録全要素に発火**するため、分割中は全ペインが各自の右端基準で設定を
  書き換え合い幅が振動していた。`ColHandle` にドラッグ元 pane の EntityId を
  持たせ、他ペインのハンドラは無視するよう修正。

### 2026-06-06 — 列幅をペイン個別に変更 ✅
- 列幅をグローバル設定 (settings) から **PaneView の `col_widths: [f32;3]`**
  (更新日時/サイズ/種類、既定 132/96/96) へ移動 — ペインごとに独立して調整可。
- セッションの `NodeData::Leaf` に `cols: Option<[f32;3]>` を追加 (後方互換 read)。
  ドラッグ → notify → app の observe → schedule_save でペイン単位に永続化。

### 2026-06-06 — タブ名にドライブ表示を必須化 ✅
- `PaneView::title`: ローカルは「C: 名前」(ドライブレター前置)、UNC は
  「🌐 名前」(share ルートは 🌐 + フルパス)。ドライブルートは「C:\」のまま。

### 2026-06-06 — タブロック ✅ (改善フェーズ一区切り)
- **中ボタンクリック**でタブのロックを切替。ロック中は × の代わりに **🔒**
  (解除も中ボタン。クリック解除は × の位置の誤操作になるため不採用)。
- ロック中は閉じられない (close_tab ガード)。
- **移動の禁止**: PaneView に `locked` を持たせ (タブの全ペインへ反映・分割時も
  継承)、`navigate()` で `OpenInNewTab(path)` を emit → app が新タブで開く。
  フォルダ進入 / ↑ / パス入力 / ツリークリック / 検索ジャンプ全てが対象。
  戻る/進む (履歴) はステータス表示のみで不動作。
- セッション: `SessionData.locked: Vec<bool>` (tabs と同列) で保存・復元。
