//! 単一行テキスト入力ウィジェット (リネーム / 新規作成ダイアログ用)。
//!
//! GPUI には標準のテキスト入力が無いため、公式サンプル
//! (vendor/crates/gpui/examples/input.rs) を移植した。
//! `EntityInputHandler` を実装しているため **IME (日本語入力) に対応**する。
//!
//! キーバインドは `bind_keys()` で `"TextInput"` コンテキスト限定で登録する。
//! これにより入力欄フォーカス時のみ有効になり、PaneView の生キー処理
//! (`on_key_down`) と干渉しない。
...
/// `"TextInput"` コンテキスト限定のキーバインドを登録する (起動時に 1 回呼ぶ)。
...
impl TextInput {
    ...
    /// 内容を設定し、指定範囲 (バイト境界) を選択状態にする。
    pub fn set_text_and_select(
        &mut self,
