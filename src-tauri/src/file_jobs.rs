...
//  - 進捗 emit を `&dyn EventSink` 経由に統一
//  - 公開純粋 API: `JobRegistry::run_copy / run_move / run_delete`
//  - `#[tauri::command]` 版は AppHandle を sink に変換して呼ぶだけ
...
fn emit_progress(sink: &dyn EventSink, p: &JobProgress) {
    events::emit(sink, "fs:job:progress", p);
}
...
fn copy_recursive(
    ...
    if meta.is_dir() {
        ...
        for ent in fs::read_dir(src)? {
            ...
            copy_recursive(&from, &to, cancel, sink, job_id, kind, c)?;
        }
    } else {
        copy_file_with_progress(src, dst, cancel, sink, job_id, kind, c)?;
    }
    Ok(())
}
...
fn delete_recursive(
    ...
    if meta.is_dir() {
        for ent in fs::read_dir(path)? {
            let ent = ent?;
            delete_recursive(&ent.path(), cancel, sink, job_id, c)?;
        }
        fs::remove_dir(path)?;
    } else {
        let sz = meta.len();
