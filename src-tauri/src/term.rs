...
//
// Phase 2A: Tauri 依存を局所化。出力 emit を `Arc<dyn EventSink>` 経由に。
...
impl TermRegistry {
    /// PTY を開き、出力読み取りスレッドを spawn して id を返す。
    /// `sink` は出力イベント (`term:data` / `term:exit`) を別スレッドから emit するために
    /// `Arc<dyn EventSink>` で受け取る。
    pub fn open_with_sink(
        ...
        let shell_path = opts.shell.unwrap_or_else(|| {
            if cfg!(windows) {
        ...
        std::thread::spawn(move || {
            ...
            loop {
                match reader.read(&mut buf) {
                    ...
                    Ok(n) => {
                        ...
                        events::emit(&*sink_for_read, "term:data", &TermDataPayload { id, data });
                    }
                    Err(_) => break,
                }
            }
            ...
            events::emit(&*sink_for_read, "term:exit", &TermExitPayload { id, code });
        });
